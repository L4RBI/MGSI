...
vec createVar(int size, float t){
  if(size == 3){
    vec var = {pow(t,2), pow(t,1), pow(t,0)};
    return var;
  }
  vec var = {pow(t,3),pow(t,2), pow(t,1), pow(t,0)};
  return var;
}
void bezierCurve(mat controlPoints, mat matrix){
  vec var;
  ...
  for(float i=0;i<=1;i=i+0.05)
  {
    ...
    mat buffer = trans(tamp) * matrix;
    mat buff = buffer * controlPoints;
...
{
  mat controlPointsQuad = {
    {0.0,1.,0.},
    {1,1.,0.},
    {-1.,0.,0.}
  };
  mat controlPointsCube= {
    {0.0,1.,0.},
    {1,1.,0.},
    {1,0.,0.},
    {1.,0.,1.}
